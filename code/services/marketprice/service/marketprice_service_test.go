...
func TestMarketPriceService_GetAverageMarketPrice(t *testing.T) {
	...
	query := "SELECT\\s+COALESCE\\(AVG\\(ad.ask_price\\), 0\\),\\s+COALESCE\\(MIN\\(ad.ask_price\\), 0\\),\\s+COALESCE\\(MAX\\(ad.ask_price\\), 0\\),\\s+COUNT\\(ad.ask_price\\).*WHERE 1=1 AND LOWER\\(b.name\\) = \\$1 AND LOWER\\(m.name\\) = \\$2"
